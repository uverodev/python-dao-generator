...
import modelo.Arqcajageneral;
import vista.*;

public class ArqcajageneralDAO extends Thread {
	private Arqcajageneral arqc;
	public ArrayList<Arqcajageneral> resultadoArqcajageneral;

	@Override
	public void run() {
		try{
			actualizarArqcajageneral("");
		} catch (Exception e) {
			System.out.println("Error al generar Arqcajageneral");
		}
		yield();
	}

	public void actualizarArqcajageneral(String condicion) {
		resultadoArqcajageneral = listar(condicion);
	}

	public ArrayList listar(String condicion) {
		try {
			inicio.gConexion.comprobar();
			String q = "select * from  arqcajageneral " + condicion + " order by id desc";
	...
	public boolean insertar(Arqcajageneral arqc) {
		String q = " insert into arqcajageneral( arq_usuario,  arq_numcaja,  arq_fecha,  arq_fechacarga,  arq_hora,  arq_cajinicial,  arq_totdescredito,  arq_totvencontado,  arq_totdescontado,  arq_totchedia,  arq_totnota,  arq_tottarcredito,  arq_comcrepagado,  arq_tottardebito,  arq_comcontado,  arq_faccrecobrado,  arq_totgasto,  arq_totvencredito,  arq_totingreso,  arq_totegreso,  arq_efecaja,  arq_borrado,  arq_totcheplazo,  arq_totefecajguarani,  arq_totefecajdolar,  arq_totefecajreal,  arq_totefecajpeso,  arq_totefecajeuro,  arq_tototrosingresos,  arq_totentregacobrada,  arq_totcuotacobrada,  arq_totnotaemitida,  arq_totsalon,  arq_totgreenfee,  arq_totcarrito,  arq_totpiscina,  arq_totocupacion,  arq_totconsumicion,  arq_totgiro,  arq_totdepbancario,  arq_tototrasentradas,  arq_totaladelanto,  arq_totalsalario,  arq_totinguarani,  arq_totindolar,  arq_totinreal,  arq_totinpeso,  arq_totineuro,  arq_totdifguarani,  arq_totdifdolar,  arq_totdifreal,  arq_totdifpeso,  arq_totdifeuro) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try {
			...
			ps.setDouble(33, arqc.getArq_totsalon());
			ps.setDouble(34, arqc.getArq_totgreenfee());
			ps.setDouble(35, arqc.getArq_totcarrito());
			ps.setDouble(36, arqc.getArq_totpiscina());
			ps.setDouble(37, arqc.getArq_totocupacion());
			ps.setDouble(38, arqc.getArq_totconsumicion());
			ps.setDouble(39, arqc.getArq_totgiro());
			ps.setDouble(40, arqc.getArq_totdepbancario());
			ps.setDouble(41, arqc.getArq_tototrasentradas());
			...
			JOptionPane.showMessageDialog(null, "Error inesperado: " + ex.getMessage(), "Agregar Arqcajageneral", JOptionPane.ERROR_MESSAGE);
		}
		return false;
	}

	public boolean modificar(Arqcajageneral arqc) {
		String q = " update arqcajageneral set  arq_usuario=?,  arq_numcaja=?,  arq_fecha=?,  arq_fechacarga=?,  arq_hora=?,  arq_cajinicial=?,  arq_totdescredito=?,  arq_totvencontado=?,  arq_totdescontado=?,  arq_totchedia=?,  arq_totnota=?,  arq_tottarcredito=?,  arq_comcrepagado=?,  arq_tottardebito=?,  arq_comcontado=?,  arq_faccrecobrado=?,  arq_totgasto=?,  arq_totvencredito=?,  arq_totingreso=?,  arq_totegreso=?,  arq_efecaja=?,  arq_borrado=?,  arq_totcheplazo=?,  arq_totefecajguarani=?,  arq_totefecajdolar=?,  arq_totefecajreal=?,  arq_totefecajpeso=?,  arq_totefecajeuro=?,  arq_tototrosingresos=?,  arq_totentregacobrada=?,  arq_totcuotacobrada=?,  arq_totnotaemitida=?,  arq_totsalon=?,  arq_totgreenfee=?,  arq_totcarrito=?,  arq_totpiscina=?,  arq_totocupacion=?,  arq_totconsumicion=?,  arq_totgiro=?,  arq_totdepbancario=?,  arq_tototrasentradas=?,  arq_totaladelanto=?,  arq_totalsalario=?,  arq_totinguarani=?,  arq_totindolar=?,  arq_totinreal=?,  arq_totinpeso=?,  arq_totineuro=?,  arq_totdifguarani=?,  arq_totdifdolar=?,  arq_totdifreal=?,  arq_totdifpeso=?,  arq_totdifeuro=? where id=?";
		...
		try {
			...
			ps.setDouble(52, arqc.getArq_totdifpeso());
			ps.setDouble(53, arqc.getArq_totdifeuro());
			ps.setInt(54, arqc.getId());
			ps.executeUpdate();
			return true;
		} catch (SQLException ex) {
			JOptionPane.showMessageDialog(null, "Error inesperado: " + ex.getMessage(), "Modificar Arqcajageneral", JOptionPane.ERROR_MESSAGE);
		}
		return false;
	}

	public boolean eliminar(Arqcajageneral arqc) {
		String q = " delete from arqcajageneral where id=?";
		try {
			inicio.gConexion.comprobar();
			PreparedStatement ps = inicio.gConexion.con.prepareStatement(q);
			...
			JOptionPane.showMessageDialog(null, "Error inesperado: " + ex.getMessage(), "Eliminar Arqcajageneral", JOptionPane.ERROR_MESSAGE);
		}
		return false;
	}
}